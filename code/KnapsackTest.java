...
public class KnapsackTest {
    ...
    public static void main(String[] args) {
        double kn_weight = MAX_WEIGHT * NUM_ITEMS * COPIES_EACH;
        for (int max_weight = 800; max_weight < 3500; max_weight += 75) {
            int w = 20000;
            double MAX_KNAPSACK_WEIGHT = max_weight * 0.4;
            ...
            FixedIterationTrainer fit = new FixedIterationTrainer(rhc, w);
            fit.train();
            System.out.printf("%d,%f,",max_weight,ef.value(rhc.getOptimal()));

            SimulatedAnnealing sa = new SimulatedAnnealing(100, .95, hcp);
            fit = new FixedIterationTrainer(sa, w);
            fit.train();
            System.out.printf("%f,",ef.value(sa.getOptimal()));

            StandardGeneticAlgorithm ga = new StandardGeneticAlgorithm(200, 150, 25, gap);
            fit = new FixedIterationTrainer(ga, w);
            fit.train();
            System.out.printf("%f,",ef.value(ga.getOptimal()));

            MIMIC mimic = new MIMIC(200, 100, pop);
            fit = new FixedIterationTrainer(mimic, w);
            fit.train();
